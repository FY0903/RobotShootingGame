...
float D_GGX(float2 m2, float NH)
{
    float f = (NH * m2 - NH) * NH + 1.0f;
...
PSOutput main(VSOutput input)
{
    ...
    float Ks = BaseColor * Metallic; // 鏡面反射係数を計算
    float a = Roughness * Roughness; // 粗さを計算
    float m2 = a * a; // 粗さの二乗を計算
    float D = D_GGX(m2, NH); // GGX分布を計算
    float G2 = G2_Smith(NL, NV, m2); // Smithのシャドウイング-マスキングを計算
